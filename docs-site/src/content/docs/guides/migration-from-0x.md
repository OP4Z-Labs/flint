...
description: What changed between Flint 0.x and 1.0, and how to upgrade existing projects.
---
...
The programmatic API (`import { ... } from '@op4z/flint'`) is new in 1.0. See [Programmatic API](/guides/programmatic-api/) for the full surface.
...
See [Telemetry transparency](/guides/telemetry-transparency/) for the full data shape and storage layout.

## Windows-native (best-effort, new in 1.0)

Flint 1.0 audited and fixed several Windows-native correctness issues — POSIX-separator manifest paths, platform-aware editor fallback, and `.cmd` shim resolution for npm/pnpm/bun/wrangler. See [Compatibility](/guides/compatibility/) for the full audit findings and known gaps.
